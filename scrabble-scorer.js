...
let newPointStructure = transform(oldPointStructure);//transform(this) variable
...
let vowelBonusScorer = function (word) {
   ...
   for (let letter of word) {//for in would return indices, for of is more readable & returns values
...
function scorerPrompt(userword) {
   ...
   let selectedScoringAlgorithm = scoringAlgorithms[userChoice];
   console.log(
      "Score for " +"'" + userword + "'" + ": " + selectedScoringAlgorithm.scorerFunction(userword));
   return selectedScoringAlgorithm;
}

function transform(oldPointStructure) {
   let transformedOldPtS = {}; // empty obj to hold obj with lowercase keys
   for (let key in oldPointStructure) {//for in iterates over keys not the values & primarily for objects not arays
      let valueArray = oldPointStructure[key];
      for (let character of valueArray) {
         character = character.toLowerCase();
         transformedOldPtS[character] = Number(key);
      }
   }
   return transformedOldPtS; // newPointStructure
}
